...
async function initDB() {
  ...
  try {
    await client.query(`
      ...
      -- PI to Courses Map (new requirement)
      ...
      ALTER TABLE program_versions ADD COLUMN IF NOT EXISTS is_rejected BOOLEAN DEFAULT false;
      ALTER TABLE program_versions ADD COLUMN IF NOT EXISTS rejection_reason TEXT;
      ALTER TABLE version_syllabi ADD COLUMN IF NOT EXISTS is_rejected BOOLEAN DEFAULT false;
      ALTER TABLE version_syllabi ADD COLUMN IF NOT EXISTS rejection_reason TEXT;
...
async function seedData(client) {
  ...
  const perms = [
    ...
    ['rbac.manage_departments', 'rbac', 'Quản lý đơn vị tổ chức'],
    ['rbac.view_audit_logs', 'rbac', 'Xem nhật ký hệ thống'],
    ['rbac.system_config', 'rbac', 'Cấu hình hệ thống'],
  ];
