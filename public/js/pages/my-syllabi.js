...
window.MySyllabiPage = {
  ...
  async loadData() {
    try {
      ...
      document.getElementById('my-syllabi-tbody').innerHTML = `<tr><td colspan="5" style="color:var(--danger);">Lỗi: ${e.message}</td></tr>`;
    }
  },
  ...
  renderTable() {
    ...
    if (this.syllabi.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5" class="text-center" style="color:var(--text-muted);padding:40px;">Bạn chưa được phân công đề cương nào.</td></tr>';
      return;
    }

    tbody.innerHTML = this.syllabi.map(s => `
      <tr style="cursor:pointer;" onclick="window.App.navigate('syllabus-editor', { syllabusId: ${s.id}, sourcePage: 'my-syllabi' })">
